...
@Slf4j
@CompileStatic
class UpdateHandler implements Closeable {
...
//    private String value
    ...
    UpdateHandler(SqlDataSource ds, Map opts) {
        this.ds = ds
        this.opts = opts ?: Collections.emptyMap()
        this.into = this.opts.into
        this.where =  this.opts.where
...
//        final result = "insert into $into ( ${columns.join(',')} ) values ( ${(QM * cnt).join(',')} )"
        final result = "update $into set ${columns.join(' = ? ,')} = ?  where $where = ?"
        return result
    }
